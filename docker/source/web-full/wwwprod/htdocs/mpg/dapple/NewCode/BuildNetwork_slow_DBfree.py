...
if len(sys.argv)>2:
	for argv in sys.argv[2:]:
		if argv.split("=")[0] == "specify":
			...
			database = argv.split("=")[1].split(",")[0]
			permuteddirectory = argv.split("=")[1].split(",")[1]
...
ppifiles = os.listdir(permuteddirectory)
...
for i in range(0,permutlength):
	if not ppifiles[i][0:2]=="RG":
		continue
	permut = buildNet(permuteddirectory+ppifiles[i],permuteddirectory+ppifiles[i]+"_dataLocAnnot",seeds,genesnp,CIcutoff,keyword=sys.argv[1])
