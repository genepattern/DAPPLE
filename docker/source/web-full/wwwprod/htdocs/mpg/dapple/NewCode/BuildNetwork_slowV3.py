...
if len(sys.argv)>2:
	for argv in sys.argv[2:]:
		...
		elif argv.split("=")[0] == "randomSet":
			randomSet = argv.split("=")[1]
...
if randomSet == "HC":
	ppifile = "/humgen/atgu1/fs03/wip/rossin/InWeb_IM/IW_HugoHC"
elif randomSet == "full":
	ppifile = "/humgen/atgu1/fs03/wip/rossin/InWeb_IM/IW_Hugo"
elif randomSet == "0.1":
	ppifile = "/humgen/atgu1/fs03/wip/rossin/InWeb_IM/IW_Hugo0.1"
...
if randomSet=="HC":
	diseaseNet = buildNet(ppifile,"/humgen/atgu1/fs03/wip/rossin/InWeb_IM/IW_HugoHC_dataLocAnnot",seeds,genesnp,CIcutoff,keyword=sys.argv[1])
elif randomSet=="full":
	diseaseNet = buildNet(ppifile,"/humgen/atgu1/fs03/wip/rossin/InWeb_IM/IW_Hugo_dataLocAnnot",seeds,genesnp,CIcutoff,keyword=sys.argv[1])
elif randomSet=="0.1":
	diseaseNet = buildNet(ppifile,"/humgen/atgu1/fs03/wip/rossin/InWeb_IM/IW_Hugo0.1_dataLocAnnot",seeds,genesnp,CIcutoff,keyword=sys.argv[1])
...
if (plot == 'true' and plotP == 'false') or (plot == 'true' and permutlength==0):
	if plot=='true':
		...
		if manualColoring == 'false':
			if plotP == 'false':
				os.system("R --vanilla --args "+collapse+" "+sys.argv[1]+" FALSE < /web/wwwprod/htdocs/mpg/dapple/NewCode/plot.R")
			else:
				os.system("R --vanilla --args "+collapse+" "+sys.argv[1]+" FALSE < /web/wwwprod/htdocs/mpg/dapple/NewCode/plotP.R")
		else:
			os.system("R --vanilla --no-save --args "+collapse+" "+sys.argv[1]+" TRUE "+manualColoring+" < /web/wwwprod/htdocs/mpg/dapple/NewCode/plot.R")
...
#ppifiles = os.listdir("/humgen/atgu1/fs03/wip/rossin/InWeb_IM/randomHC/")
if randomSet == "full":
	ppifiles = os.listdir("/humgen/atgu1/fs03/wip/rossin/InWeb_IM/randomFull/")
	ppifilesDir = "/humgen/atgu1/fs03/wip/rossin/InWeb_IM/randomFull/"
elif randomSet == "HC":
	ppifiles = os.listdir("/humgen/atgu1/fs03/wip/rossin/InWeb_IM/randomHC/")
	ppifilesDir = "/humgen/atgu1/fs03/wip/rossin/InWeb_IM/randomHC/"
elif randomSet == "0.1":
	ppifiles = os.listdir("/humgen/atgu1/fs03/wip/rossin/InWeb_IM/random0.1/")
	ppifilesDir = "/humgen/atgu1/fs03/wip/rossin/InWeb_IM/random0.1/"
...
for i in range(0,permutlength):
	...
	permut = buildNet(ppifilesDir+ppifiles[i],ppifilesDir+ppifiles[i]+"_dataLocAnnot",seeds,genesnp,CIcutoff,keyword=sys.argv[1])
...
for seed in seedsCopy:
	...
	if seed in diseaseNet["seedDirectDegrees"].keys():
		if not seed in seedDirectDegreesPermut.keys():
			...
			countfile.write(seed+"\t0\t")
		else:
			pdirect = (float(seedDirectDegreesPermut[seed])+1)/(float(permutlength)+1)
			countfile.write(seed+"\t"+str(seedDirectDegreesPermut[seed])+"\t")
	...
	outfile.write(seed+"\t"+genesnpCopy[seed]+"\t"+str(pfinal)+"\t"+str(pfinalCorrected)+"\t"+str(correctby1)+"\t"+str(correctby2)+"\n")
...
if plot == 'true' and plotP == 'true':
	...
	if plot=='true':
		...
		for seed in genesnpCopy.keys():
			...
			colormap[seed] = int(snps.index(genesnpCopy[seed]))
		outfile.close()
		if collapseCI == 'false':
			collapse = "FALSE"
		else:
...
while len(genestospecify) > genesToSpecifyCount:
	...
	diseaseNet = buildNet(ppifile,"/home/unix/rossin/DALY/PPI/NewCode/ppiannot",seeds,genesnp,CIcutoff,keyword=sys.argv[1])
