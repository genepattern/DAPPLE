...
source("/code/3Dcircle.R")
source("/code/collapseCI.R")
...
#		tmp[in.set.ppi[2,1],in.set.ppi[2,2]]=tmp[in.set.ppi[2,2],in.set.ppi[2,1]]=1
	...
	if(manual.coloring==FALSE) {
		...
		legendcols = hsv(colorpalette[as.numeric(unique(vertex.col))],.4,1);
		legendlabels = unlist(lapply(as.numeric(unique(vertex.col)),function(x){return(SNP.gene[which(SNP.gene[,3]==x),1][1])}))
	}
	if(manual.coloring==TRUE) {
