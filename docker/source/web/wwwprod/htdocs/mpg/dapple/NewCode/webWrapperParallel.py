...
if permute<1000:
	...
	if not genestospecify=='false':
		os.system("python /web/wwwprod/htdocs/mpg/dapple/NewCode/BuildNetwork_slow.py "+seedFile+" permute="+str(permute)+" CIcutoff="+str(CIcutoff)+" plot="+str(plot)+" iterate="+iterate+" compare=true"+" specify="+genestospecify+" keyword="+keyword+" dir="+dir+" nearestgene="+nearestgene+" regulatory="+regulatory)
	else:
		os.system("python /web/wwwprod/htdocs/mpg/dapple/NewCode/BuildNetwork_slow.py "+seedFile+" permute="+str(permute)+" CIcutoff="+str(CIcutoff)+" plot="+str(plot)+" iterate="+iterate+" compare=true keyword="+keyword+" dir="+dir+" nearestgene="+nearestgene+" regulatory="+regulatory)
else:
	##Figure out working directory
	tmpdir = seedFile.split("/")
	dir	= "/".join(tmpdir[0:8])
	...
	##get job ID
	jobid = open("/"+tmpdir[-3]+"/statusUpdate",'r').readlines()[0].strip("\n")
	
	if not genestospecify=='false':
		os.system("python /web/wwwprod/htdocs/mpg/dapple/NewCode/BuildNetwork_slow.py "+seedFile+" permute="+str(subPermute)+" CIcutoff="+str(CIcutoff)+" plot="+str(plot)+" iterate="+iterate+" compare=true"+" specify="+genestospecify+" keyword="+keyword+" dir="+dir+" nearestgene="+nearestgene+" regulatory="+regulatory)
	else:
		os.system("python /web/wwwprod/htdocs/mpg/dapple/NewCode/BuildNetwork_slow.py "+seedFile+" permute="+str(subPermute)+" CIcutoff="+str(CIcutoff)+" plot="+str(plot)+" iterate="+iterate+" compare=true keyword="+keyword+" dir="+dir+" nearestgene="+nearestgene+" regulatory="+regulatory)
