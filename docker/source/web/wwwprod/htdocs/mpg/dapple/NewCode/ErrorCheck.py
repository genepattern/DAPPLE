...
##Create a 2nd outfile to decide whether to use hour or week queue
outfile2 = open("whichQ",'w')
...
if inputlist[0][0:2]=="rs" and len(inputlist[0].split())!=4:
	...
	###check that there are no more than 500 snps
	if len(inputlist) > 2000:
		outfile = open("ErrorWarningFile.txt",'w')
		os.system("chmod 777 ErrorWarningFile.txt")
		outfile.write("error\n")
		outfile.write("The maximum number of SNPs to input is 300.")
		outfile.close()
		sys.exit()
...
##if input is a list of genes
	...
	if len(inputlist) > 2000:
		...
		os.system("chmod 777 ErrorWarningFile.txt")
		outfile.write("error\n")
		outfile.write("Please input fewer than 500 entries.")
		outfile.close()
		sys.exit()
	if len(inputlist)==1:
		outfile = open("ErrorWarningFile.txt",'w')
...
##if input is a list of regions
	...
	for line in inputlist:
		...
		if not line[2].isdigit() or not line[2].isdigit():
			...
			outfile.close()
			sys.exit()
	if len(inputlist) > 1000:
		outfile = open("ErrorWarningFile.txt",'w')
		os.system("chmod 777 ErrorWarningFile.txt")
		outfile.write("error\n")
		outfile.write("Please do not enter more than 500 regions.")
		outfile.close()
		sys.exit()
	if len(inputlist) == 1:
		outfile = open("ErrorWarningFile.txt",'w')
...
if float(len(seeds))/float(len(inputlist)) < 0.1 or len(seeds)==1:
	outfile = open("ErrorWarningFile.txt",'w')
	os.system("chmod 777 ErrorWarningFile.txt")
	outfile.write("error\n")
